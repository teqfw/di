...
/**
 * @typedef {{depId: TeqFw_Di_Dto_DepId, context: TeqFw_Di_Container_ResolutionContext, namespace: object|null, dependencies: Map<string, string>, mock: {found: boolean, value: unknown}}} TeqFw_Di_Container_GraphResolver_Node
 */
...
export default class TeqFw_Di_Container_GraphResolver {
    ...
    constructor({canonicalize, parser, findMock = () => ({found: false, value: undefined}), resolver, logger = null}) {
        /** @type {(specifier: string, ancestors: readonly TeqFw_Di_Dto_DepId[]) => TeqFw_Di_Dto_DepId} */
        const canonicalizeFn = canonicalize ?? function (specifier) {
         ...
         */
        const walk = async function (depId, context, out, stack, chain) {
            /** @type {string} */
            ...
            if (mock.found) {
                out.set(key, {depId, context, namespace: null, dependencies: new Map(), mock});
                return;
            }
            ...
            try {
                ...
                for (const [name, nextSpecifier] of Object.entries(depsMap)) {
                    ...
                    const nextDepId = canonicalizeFn(/** @type {string} */ (nextSpecifier), context.stack);
                    /** @type {TeqFw_Di_Container_ResolutionContext} */
                    const nextContext = createResolutionContext(nextDepId, context.stack);
                    ...
                    await walk(nextDepId, nextContext, out, stack, chain);
                }
            } finally {
                chain.pop();
        ...
        this.resolve = async function (depId) {
            ...
            /** @type {string[]} */
            const chain = [];
            await walk(depId, createResolutionContext(depId, []), out, stack, chain);
            return out;
        };
    }
}
