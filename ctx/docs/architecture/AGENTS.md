...
- `constraints.md` — non-negotiable architectural constraints and stability boundary of the architectural branch.
- `depid-model.md` — declarative model of structural canonical dependency identity (`DepId`) and its immutable invariants.
- `execution-model.md` — container execution model: lifecycle states, request semantics, determinism across time, and failure behavior.
